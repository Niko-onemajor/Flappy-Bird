...
export default class Saw extends Sprite {
  ...
  init(speed, pipe, prevPipe) {
    ...
    console.log(`[Saw] 生成 x=${this.x.toFixed(1)} y=${this.y.toFixed(1)} prevRight=${prevPipe ? prevRight.toFixed(1) : 'none'} currLeft=${currLeft.toFixed(1)} gapSpace=${gapSpace.toFixed(1)} pipe(x=${pipe.x.toFixed(1)},gapY=${pipe.gapY.toFixed(1)},gap=${pipe.gap},type=${pipe.pipeType})`);
  }
